            ...
            for k in range(len(gt_keys)):
                ...
                for i in range(len(eval_percentages)):
                    ...
                    for s in range(test_num_samples):
                        ...
                        moc = np.mean(classes_n_T[count_mask] / (classes_n_T[count_mask] + classes_n_F[count_mask]))
                        if moc > max_moc:
                            max_moc = moc
            ...
            for j in range(len(eval_percentages)):
                ...
                for i in range(self.num_classes):
                    if n_T_classes_all_files[j, i] + n_F_classes_all_files[j, i] != 0:
                        itm = float(n_T_classes_all_files[j, i]) / (n_T_classes_all_files[j, i] + n_F_classes_all_files[j, i])
                        acc += itm
                        n += 1

                    if (max_n_T_classes_all_files[j, i] + max_n_F_classes_all_files[j, i] != 0):
                        itm_max = float(max_n_T_classes_all_files[j, i]) / (
